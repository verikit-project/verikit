...
/**
 * Permissions fail closed: an operation, field, or action with no matching rule is denied until the caller explicitly attaches an allow rule.
 */
...
/**
 * Checks whether the actor in `context` has read or write access to a named field. Resolves to `{ allowed: false }`
 * if no rule was attached via `.field()` for that field/access combination. Read and write are independent:
 * a `.field(name, { read })` call leaves write access denied until its own rule is set (and vice versa).
 */
...
/**
 * Checks whether the actor in `context` may run a named runtime action. Resolves to `{ allowed: false }` if no rule was attached via `.action()` for that action name.
 */

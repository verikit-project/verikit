...
/**
 * Matches the segments remaining after a resource's base path against the
 * seven routes `createServer` exposes for that resource, given the request
 * method. Distinguishes "no such shape" (404) from "right shape, wrong verb"
 * (405) so the caller can respond accordingly.
 */
